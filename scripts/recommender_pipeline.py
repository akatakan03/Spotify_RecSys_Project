...
class SpotifyRecommenderPipeline:
    def __init__(self):
        ...
        # 4. Ses Özellikleri İçin Standardization (Ölçeklendirme)
    ...
    def recommend_hybrid(self, seed_uris, w_audio=0.5, w_co=0.5, top_n=10):
        ...
        # 3. BOYUT HİZALAMA (Dimensionality Alignment) - Hatanın Çözümü
        # df_merged sırasındaki şarkıların matris indekslerini alıp, co_scores'u filtrelenmiş sıraya diziyoruz
        merged_uris = self.df_merged['track_uri'].tolist()
        merged_matrix_indices = [self.uri_to_idx[uri] for uri in merged_uris]
        co_scores = co_scores_full[merged_matrix_indices] # Boyut artık tam olarak 4939 oldu!
        
        if co_scores.max() > 0:
            co_scores = co_scores / co_scores.max()
        
        # 4. Hybrid Equation (Artık iki dizi de aynı boyutta ve sırada toplanabilir)
        final_scores = (audio_sims * w_audio) + (co_scores * w_co)
        results_idx = np.argsort(final_scores)[::-1]
        
        oneriler = []
        for idx in results_idx:
            uri = self.df_merged.iloc[idx]['track_uri'] # Doğru index eşleşmesi (Index Matching)
...
if __name__ == "__main__":
    ...
    print("TEST 1: Hibrit Motor")
    test_seeds = pipeline.df_metadata['track_uri'].head(3).tolist()
    res1 = pipeline.recommend_hybrid(test_seeds, top_n=3)
    for r in res1: print(f"- {r['name']} by {r['artist']} (Skor: {r['score']:.2f})")
    
    print("\nTEST 2: Senaryo Motoru")
    res2 = pipeline.recommend_scenario("gym workout fast", top_n=3)
    for r in res2: print(f"- {r['name']} by {r['artist']} (Skor: {r['score']:.2f})")